...
export default {
  ...
  async mounted () {
    const response = await axios.get('api/bucketListItems.js')
    this.items = response.data;
  },
  methods: {
    async addItem () {
      console.log('description:', this.description);
      const response = await axios.post("api/bucketListItems.js", {
    ...
    async removeItem (item, i){
      await axios.delete("api/bucketListItems.js/" + item._id);
  ...
  select(item) {
    ...
    async updateItem(item, i) {
      const response = await axios.put("api/bucketListItems.js/" + item._id, {
